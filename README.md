...
| `who [--repo] [--all] [--coverage [--cwd P \| --under P]]` | sessions with computed status; coverage = live root `claude`/`codex` processes, `unenrolled` roots (each live session enrolls only its nearest root of its own kind, in scope, at its pid or below it with a verified `pid_start`), and `complete` when none are unenrolled -- those two are authoritative, `enrolled` counts are informational; `--cwd` scopes to exactly P, `--under` to P and below (unreadable cwd still counts) |
