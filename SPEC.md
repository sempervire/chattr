...
| `who [--repo] [--all] [--coverage [--cwd P \| --under P]]` | `sessions: [Session]`; with `--coverage`: `coverage: {processes:{claude,codex}, enrolled:{claude,codex}, unenrolled:[{pid, kind, cwd}], complete: bool}`; `--cwd` counts only processes and sessions in directory P; `--under` counts those in P or below it (path-segment match on realpaths); either way a process whose cwd is unreadable still counts. A countable session process is a root: a `claude` or `codex` process whose parent is not a same-named session process. `app-server` and `sandbox` subcommands are not session processes: a codex process whose subcommand (the first argument after the executable that is not a `-c <key=value>` pair, from a second `ps -ww -Ao pid=,args=` scan) is one of them is never a root, so `processes`/`unenrolled` never include it, and a codex it launches is a root. Each live session enrolls at most one root: the nearest root at or above its pid, and only when that root's name equals the session's `kind`, the session's own cwd is in the same `--cwd`/`--under` scope, and either its pid is the root's or its row has a `pid_start` matching the live process. `unenrolled` lists each counted root no session enrolls (`cwd` null when unreadable); `complete` is true only when `ps` succeeds and `unenrolled` is empty. `complete` and `unenrolled` are authoritative; `enrolled` (live sessions per kind in scope) is informational only. A root the args scan doesn't answer, or that isn't positively identified as one of them, still counts (fail closed); a `ps` failure gives `complete: false`. Accepted limitation: an unenrolled conversation hosted inside an app-server (e.g. the VS Code OpenAI extension or the ChatGPT app) is invisible to coverage -- only its own enrollment, never process presence, can surface it, and it never makes coverage incomplete |
